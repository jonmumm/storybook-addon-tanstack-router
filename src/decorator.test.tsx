...
describe("withTanStackRouter", () => {
	it("renders a story with default router context (no config)", async () => {
		...
		callDecorator(Story);
		expect(await screen.findByText("Default Router")).toBeInTheDocument();
	});
	...
	it("provides route params to the story", async () => {
		...
		expect(await screen.findByText("User ID: 42")).toBeInTheDocument();
	});
	...
	it("provides search params to the story", async () => {
		...
		expect(await screen.findByText("Tab: settings")).toBeInTheDocument();
	});
	...
	it("provides both params and search together", async () => {
		...
		expect(await screen.findByText("User: 42, Tab: settings")).toBeInTheDocument();
	});
});
